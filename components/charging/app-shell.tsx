...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div
      className="mx-auto flex min-h-screen w-full max-w-[420px] flex-col"
      style={{ backgroundColor: "var(--brand-app-bg)" }}
    >
      {showChrome ? (
        <header className="sticky top-0 z-30 flex items-center justify-between gap-2 border-b border-[var(--brand-hairline,#e3e7f2)] bg-white/95 px-4 py-3 backdrop-blur">
      ...
      <main className={`flex-1 ${showChrome ? "pb-24" : ""}`}>
        {!hydrated ? (
      ...
      {showChrome ? (
        <nav
          ...
          className="fixed bottom-0 left-1/2 z-40 w-full max-w-[420px] -translate-x-1/2 border-t border-[var(--brand-hairline,#e3e7f2)] bg-white/95 px-2 pb-[max(0.5rem,env(safe-area-inset-bottom))] pt-1 backdrop-blur"
        >
